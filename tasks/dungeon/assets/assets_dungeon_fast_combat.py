from module.base.button import Button, ButtonWrapper

# This file was auto-generated, do not modify it manually. To generate:
# ``` python -m dev_tools.button_extract ```

FAST_COMBAT_LOCKED = ButtonWrapper(
    name='FAST_COMBAT_LOCKED',
    share=Button(
        file='./assets/share/dungeon/fast_combat/FAST_COMBAT_LOCKED.png',
        area=(1220, 636, 1263, 680),
        search=(1200, 616, 1280, 700),
        color=(111, 69, 30),
        button=(1220, 636, 1263, 680),
    ),
)
FAST_COMBAT_OFF = ButtonWrapper(
    name='FAST_COMBAT_OFF',
    share=Button(
        file='./assets/share/dungeon/fast_combat/FAST_COMBAT_OFF.png',
        area=(1221, 642, 1257, 683),
        search=(1201, 622, 1277, 703),
        color=(47, 45, 48),
        button=(1221, 642, 1257, 683),
    ),
)
FAST_COMBAT_ON = ButtonWrapper(
    name='FAST_COMBAT_ON',
    share=Button(
        file='./assets/share/dungeon/fast_combat/FAST_COMBAT_ON.png',
        area=(1221, 643, 1258, 684),
        search=(1201, 623, 1278, 704),
        color=(152, 154, 154),
        button=(1221, 643, 1258, 684),
    ),
)
FAST_COMBAT_RESULT_CLOSE = ButtonWrapper(
    name='FAST_COMBAT_RESULT_CLOSE',
    cn=Button(
        file='./assets/cn/dungeon/fast_combat/FAST_COMBAT_RESULT_CLOSE.png',
        area=(835, 548, 869, 566),
        search=(815, 528, 889, 586),
        color=(152, 144, 137),
        button=(835, 548, 869, 566),
    ),
    global_cn=Button(
        file='./assets/global_cn/dungeon/fast_combat/FAST_COMBAT_RESULT_CLOSE.png',
        area=(834, 548, 870, 566),
        search=(814, 528, 890, 586),
        color=(146, 138, 130),
        button=(834, 548, 870, 566),
    ),
    global_en=None,
)
FAST_COMBAT_TIMES_MINUS = ButtonWrapper(
    name='FAST_COMBAT_TIMES_MINUS',
    share=Button(
        file='./assets/share/dungeon/fast_combat/FAST_COMBAT_TIMES_MINUS.png',
        area=(1222, 556, 1245, 577),
        search=(1202, 536, 1265, 597),
        color=(64, 63, 65),
        button=(1222, 556, 1245, 577),
    ),
)
FAST_COMBAT_TIMES_PLUS = ButtonWrapper(
    name='FAST_COMBAT_TIMES_PLUS',
    share=Button(
        file='./assets/share/dungeon/fast_combat/FAST_COMBAT_TIMES_PLUS.png',
        area=(946, 562, 967, 583),
        search=(926, 542, 987, 603),
        color=(53, 51, 55),
        button=(946, 562, 967, 583),
    ),
)
FAST_COMBAT_WINDOW = ButtonWrapper(
    name='FAST_COMBAT_WINDOW',
    share=Button(
        file='./assets/share/dungeon/fast_combat/FAST_COMBAT_WINDOW.png',
        area=(150, 135, 179, 168),
        search=(130, 115, 199, 188),
        color=(95, 95, 95),
        button=(150, 135, 179, 168),
    ),
)
OCR_FAST_COMBAT_CURRENT_TIMES = ButtonWrapper(
    name='OCR_FAST_COMBAT_CURRENT_TIMES',
    share=Button(
        file='./assets/share/dungeon/fast_combat/OCR_FAST_COMBAT_CURRENT_TIMES.png',
        area=(1061, 551, 1128, 582),
        search=(1041, 531, 1148, 602),
        color=(71, 70, 74),
        button=(1061, 551, 1128, 582),
    ),
)
OCR_FAST_COMBAT_REMAINING_TIMES = ButtonWrapper(
    name='OCR_FAST_COMBAT_REMAINING_TIMES',
    share=Button(
        file='./assets/share/dungeon/fast_combat/OCR_FAST_COMBAT_REMAINING_TIMES.png',
        area=(1222, 497, 1251, 519),
        search=(1202, 477, 1271, 539),
        color=(80, 80, 83),
        button=(1222, 497, 1251, 519),
    ),
)
