...
from tasks.knights_v2.assets.assets_knights_v2_activity_support_passive import (
    REQUEST_ACTION,
...
class KnightsV2SupportMixin:
    ...
    def _run_support_donate(self, skip_first_screenshot=True) -> bool:
        ...
        if not plans:
            logger.info("Knights v2 support donate disabled by config")
            return True

        logger.info("Knights v2 support: donate")
        ...
        while 1:
            ...
            if timeout.reached():
                logger.warning("Knights v2 support donate timeout, stop scanning")
                return True

            if not self.appear(SUPPORT_CHECK):
                if self.appear(page_knights_v2.check_button):
                    logger.warning("Knights v2 support donate exited to knights page unexpectedly")
                    return False
                if self.is_in_main(interval=0):
                    logger.warning("Knights v2 support donate exited to main page unexpectedly")
                    return False
            ...
            if action_buttons or complete_buttons:
                ...
                if not empty_list_confirm.started():
                    logger.info("Knights v2 support donate: no visible entries, confirm empty list")
                    empty_list_confirm.start()
                elif empty_list_confirm.reached():
                    logger.info("Knights v2 support donate list is empty, skip scan")
                    return True
                continue
            ...
            if pairs:
                ...
                logger.info(f"Knights v2 support donate: {plan_name} -> {action_button}")
            ...
            if complete_count >= self.SUPPORT_DONATE_COMPLETE_COUNT:
                logger.info(f"Knights v2 support donate completed on current page: {complete_count}")
                return True
            ...
            if skip_hit_bottom_once:
                ...
                if self.appear(HIT_BOTTOM):
                    ...
                    if hit_bottom_confirm >= 2:
                        logger.info("Knights v2 support donate reached bottom")
                        return True
                else:
                    hit_bottom_confirm = 0
    ...
    def _run_support_request(self, skip_first_screenshot=True) -> bool:
        ...
        if not plan or not plan.enabled:
            logger.info("Knights v2 support request disabled by config")
            return True

        logger.info("Knights v2 support: request")
        ...
        while 1:
            ...
            if timeout.reached():
                logger.warning("Knights v2 support request timeout")
                return True

            if submitted and self.appear(SUPPORT_CHECK):
                logger.info("Knights v2 support request finished")
                return True
            ...
            if not panel_opened:
                ...
                if self.appear(SUPPORT_CHECK) and open_retry.reached():
                    logger.info("Knights v2 support request unavailable, skip")
                    return True
                continue
            ...
            if not selected:
                if plan.selected_asset and self.appear(plan.selected_asset):
                    selected = True
                    logger.info(f"Knights v2 support request selected: {plan.name}")
                    timeout.reset()
                    continue
                ...
                if choose_retry.reached():
                    logger.warning("Knights v2 support request item selection not confirmed, skip")
                    return True
                continue
            ...
            if self.appear(SUPPORT_CHECK):
                logger.info("Knights v2 support request returned to support page")
                return True

    def _back_to_knights_from_support(self, skip_first_screenshot=True) -> bool:
        self.ui_goto(page_knights_v2, skip_first_screenshot=skip_first_screenshot)
        return True

    def run_support(self, skip_first_screenshot=True, run_donate=True, run_request=True) -> bool:
        logger.hr("Knights V2 Support", level=2)
        ...
        if run_request:
            if success:
                ...
                logger.warning("Knights v2 support request skipped due to donate flow failure")
